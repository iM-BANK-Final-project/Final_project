...
`web_m12_intervene_v2_scores_202512_all_3372.csv`는 2025-12의 3,372개 법인을 모두 포함한다. 서비스는 `score_eligible=True`인 정확히 3,341개만 사용한다. 부적격 31개는 DB, API, KPI, 필터, 화면에서 제외한다.

점수 artifact의 현재 세그먼트, 세그먼트 전이, D/A/C/K 변화율, 상위 3개 SHAP 설명을 서비스에 저장한다. 웹에서는 모델이나 SHAP를 재계산하지 않는다.
