...
기준월에 이미 `core_3m_event=1`이면 신규 조기경고 대상이 아니므로 모델링 패널에서 제외한다. 현재 1~2개월 연속 drop50은 기준월까지 관측된 합법적인 조기신호이므로 feature로 유지한다. 과거 사건에 대한 추가 cooldown은 baseline 범위에서 적용하지 않고 반복 알림 분석 결과를 보고 결정한다.
...
5. 현재 `core_3m_event=1` 기준월은 학습 패널에서 제외된다.
...
실제 원천 데이터가 작업공간에 없으면 구현과 합성 테스트까지만 완료하고 성능 수치를 생성하거나 문서에 주장하지 않는다.
...
- 실제 데이터 실행 전에는 모델 성능을 공란 또는 미측정으로 유지한다.
